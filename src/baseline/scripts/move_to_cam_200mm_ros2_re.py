...
class PiperMoveToObservation(Node):
    def __init__(self):
        ...
        # [SRDF 명칭 적용] 그룹, 기준 링크, 끝단 링크
        self.group_name = 'right_arm'
        self.base_frame = 'right_base_link'
        self.ee_link = 'right_link6'
    ...
    def get_target_pose_matrix(self, dist_m, pitch_deg, yaw_deg, roll_deg):
        ...
        # 로봇 베이스(right_base_link) 기준 카메라의 위치 및 방향 변환 행렬
        T_base_cam = np.array([
    ...
    def execute_move(self):
        ...
        self.get_logger().info("카메라 정면 200mm(0.2m) 관측 지점의 IK를 계산합니다...")

        # --- [★ 핵심 수정: 카메라 기준 0.4m(400mm) 앞 좌표 및 자세 계산] ---
        # dist_m = 0.4 (40cm)으로 변경
        target_matrix = self.get_target_pose_matrix(dist_m=0.2, pitch_deg=0.0, yaw_deg=0.0, roll_deg=0.0)
        
        # 변환 행렬에서 위치(Position) 추출
        target_pos = target_matrix[:3, 3]
        
        # 변환 행렬에서 회전(Orientation) 추출 후 쿼터니언 변환 (x, y, z, w 순서)
        rot_matrix = target_matrix[:3, :3]
        target_quat = R.from_matrix(rot_matrix).as_quat()

        self.get_logger().info(f" -> 계산된 목표 위치(Base 기준): X={target_pos[0]:.3f}, Y={target_pos[1]:.3f}, Z={target_pos[2]:.3f}")
        ...
        # 계산된 목표 회전(쿼터니언) 대입
        req.ik_request.pose_stamped.pose.orientation.x = float(target_quat[0])
        req.ik_request.pose_stamped.pose.orientation.y = float(target_quat[1])
        req.ik_request.pose_stamped.pose.orientation.z = float(target_quat[2])
        req.ik_request.pose_stamped.pose.orientation.w = float(target_quat[3])
        ...
        if res.error_code.val != 1:
            ...
            self.get_logger().error("팁: 로봇이 도달할 수 없는 작업 영역이거나 각도 제한에 걸렸을 수 있습니다. pitch/yaw/roll 각도를 조정해보세요.")
            return
        ...
        if result.error_code == 0:
            self.get_logger().info("===== [2. Move 완료] 카메라 앞 400mm(0.4m) 작업 위치 도달 성공! =====")
        else:
